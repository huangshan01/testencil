...
int main(int argc, char * argv[]){
	...
	Bx = ceild(ix + 2*tb*XSLOPE,2);
	By = ceild(iy + 2*tb*YSLOPE,2);
	bx = Bx - tb*XSLOPE*2;
	by = By - tb*YSLOPE*2;
	ix = Bx + bx;
	iy = By + by;
	...
	if(tb < 0 || Bx <= bx){
		if(iam == 0){
			printf("NX = %d\tNY = %d\tBx = %d\tbx = %d\tBy = %d\tby = %d\ttb = %d\tERROR!\n",NX,NY,Bx,bx,By,by,tb);
		}
		return 0;
	}
	...
	DATA_TYPE *sbuf1 = (DATA_TYPE *)malloc(sizeof(DATA_TYPE)*(iy+tb+YSLOPE)*(tb+XSLOPE)*2);//t=0,t=1
	...
	int ybottom02[2] = {YSLOPE, YSLOPE+iy/2};
...
#pragma simd
					for(x = xmin; x < xmax; x++){
						kernel(A);
						//if(iam==0) printf("%d\tt=%d\tB0B2\t(%d,%d)\n",iam,t,x,y);
					}
				}
			}
		}
		...
		//if(iam==2) printf("before b1, A[0][8][7]=%lf\tA[1][8][7]=%lf\tB[0][8][7]=%lf\tB[1][8][7]=%lf\n",A[0][8][7],A[1][8][7],B[0][8][7],B[1][8][7]);
...
#pragma simd
		...
		if((iam%npx) & 0x1){//奇数列进程先发送sbuf1
			...
			if((iam+1)%npx != 0){//除每行最右进程
				...
				//if(iam==0) printf("%d:\t level=%d \t B1 rbuf1 recv!\n",iam,level);
			}
			if(iam%npx != 0){//除每行最左进程
		...
		if((iam/npx) & 0x1){//奇数行进程先发送sbuf2
			...
			if(iam < npx*(npy-1)){//除每列最上进程
				...
				//if(iam==0) printf("%d:\t level=%d \t B1 rbuf2 %d recv!\n",iam,level,count2);
			}
			if(iam >= npx){//除每列最下进程
		...
		if((iam+1)%npx != 0){//除了每行最右进程, 解包rbuf1(横向右侧接收)
			//printf("%d:\t level=%d \t rbuf1 unpack begin!\n",iam,level);
			count1 = 0;
			for(t = 0; t < 2; t++){
				for(i = XSLOPE+ix; i < XSLOPE+ix+tb+XSLOPE; i++){
		...
		if(iam < npx*(npy-1)){//除了最上层进程，解包rbuf2(纵向上侧接收)
			...
			//printf("%d:\t level=%d \t rbuf2 unpack complete!\n",iam,level);
		}
		}
		else{ //level == 1
		if((iam+1)%npx != 0){//除了每行最右进程, 打包sbuf1(横向向右传输)
